...
describe('extractWordleId', () => {
  ...
  it('should throw an error for invalid Wordle strings', () => {
    expect(() => extractWordleId('Word 123')).toThrow(
      'No wordle id found in Word 123'
    )
    expect(() => extractWordleId('Wordle123')).toThrow(
      'No wordle id found in Wordle123'
    )
  })
  ...
  it('should throw an error for strings without a number', () => {
    expect(() => extractWordleId('Wordle results for')).toThrow(
      'No wordle id found in Wordle results for'
    )
  })
})
...
describe('extractWordleScore', () => {
  ...
  it('should throw an error for invalid score lines', () => {
    expect(() => extractWordleScore('Wordle 123')).toThrow(
      'No wordle score found in "Wordle 123"'
    )
...
describe('parseWordleRecord', () => {
  ...
  it('should throw an error for an invalid Wordle record message', () => {
    ...
    expect(() => parseWordleRecord(msg, client as any)).toThrow(
      'Invalid wordle record'
    )
  })
  ...
  it('should throw an error when no wordle score is found', () => {
    ...
    expect(() => parseWordleRecord(msg, client as any)).toThrow(
      'No wordle score found in "1. by <@123> (hard mode)"'
    )
  })
  ...
  it('should throw an error when no username is found for a user id', () => {
    ...
    const client = {
      users: {
        cache: {
          get: (id: string) =>
            id === '789' ? null : { username: `user${id}` },
        },
      },
    }

    expect(() => parseWordleRecord(msg, client as any)).toThrow(
      'No username found for user id 789'
    )
  })
  ...
  it('should throw an error when no guesses are found for a user', () => {
    ...
    expect(() => parseWordleRecord(msg, client as any)).toThrow(
      'No guesses found for user user456'
    )
  })
})

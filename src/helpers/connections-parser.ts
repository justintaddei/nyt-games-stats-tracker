...
type ConnectionsMessage = { // TODO: move this to types.ts and use for both wordle and connections
    content: string;
    author: {
        username: string;
        id: string;
...
export const isConnections = (text: string = ''): boolean => /^Connections\s+Puzzle\s+#\d+$/m.test(text)
export const isConnectionsRecord = (text: string = ''): boolean => /^Connections results for Puzzle #\d+:$/m.test(text)
...
export const computeConnectionsScore = (lines: string[]): ConnectionsScore => {
    ...
    const isCorrect = (line: string) => splitEmoji(line).every((char, _, l) => {
        return char === l[0]
    })
...
export const parseConnectionsRecord = (msg: ConnectionsMessage, client: Client): ConnectionsRecord => {
    ...
    const mappedAttempts = lines.slice(attemptsStart).reduce((_attempts, line) => {
        if (line.startsWith("@")) {
